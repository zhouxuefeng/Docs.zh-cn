...
若要将 HTTP 请求重定向到 HTTPS，请参阅[URL 重写中间件](xref:fundamentals/url-rewriting)。 如果你使用的 Visual Studio Code 或在本地平台上测试，不包括测试证书用于 SSL:
...
添加`ContactOperations`类到*授权*文件夹。 此类包含要求我们的应用程序支持：
